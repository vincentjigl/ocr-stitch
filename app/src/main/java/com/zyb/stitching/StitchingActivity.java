...
public class StitchingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // mTextViewJni.setText(NativeStitch.getMessageFromJni());
        ...
        Runnable imageProcessingRunnable = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Log.i("Stitching","elems " +  elems);

                    long[] tempobjadr = new long[elems];
                    for (int i = 0; i < elems; i++) {
                        tempobjadr[i] = listImage.get(i).getNativeObjAddr();
                        Log.i("Stitching","tempobjadr["+i+"] " +  tempobjadr[i]);

                    }

                    // Create a Mat to store the final Stitching image
                    Mat result = new Mat();
                    // Call the OpenCV C++ code to perform stitching process
                    int ret = NativeStitching.processStitching(tempobjadr, result.getNativeObjAddr());
                    if(ret==0){
                        Log.i("Stitching","ret " +  ret);
                    }
                    else{
                        Log.i("Stitching","ret " +  ret);
                    }
                    ...
                    try {
                        ...
                        if (bool)
                            Log.i("Stitching","SUCCESS writing image to external storage" +  fileName);
                        else
                            Log.i("Stitching", "Fail writing image to external storage" + fileName);
            ...
            private void showProcessingDialog() {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        mCam.stopPreview();
                        ringProgressDialog = ProgressDialog.show(StitchingActivity.this, "", "Stitching", true);
